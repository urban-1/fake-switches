...
class NetconfProtocolTest(unittest.TestCase):
    ...
    def test_close_session_support(self):
        ...
        self.netconf.dataReceived(
            b'<nc:rpc xmlns:nc="urn:ietf:params:xml:ns:netconf:base:1.0" message-id="12345">\n'
        )
    ...
    def test_get_config_support(self):
        self.netconf.datastore.set_data(
            RUNNING, {"configuration": {"stuff": "is cool!"}}
        )
        self.netconf.connectionMade()
        self.say_hello()
    ...
    def test_edit_config(self):
        self.netconf.datastore.set_data(
            RUNNING, {"configuration": {"stuff": {"substuff": "is cool!"}}}
        )
        self.netconf.connectionMade()
        self.say_hello()
    ...
    def test_filtering(self):
        ...
        assert_that(content.xpath("//data/configuration/shizzle/*"), has_length(2))
        assert_that(
            content.xpath("//data/configuration/shizzle/whizzle/howdy"), has_length(1)
        )
    ...
    def test_filtering_with_a_value(self):
        ...
        assert_that(
            content.xpath("//data/configuration/element/attribute/*"), has_length(1)
        )
        ...
        content = dict_2_etree(
            {
                "data": {
                    "configuration": {
                        "interfaces": {
                            "interface": [{"name": "irb"}, {"unit": {"name": "123"}}]
                        }
                    }
                }
            }
        ...
        content_filter = dict_2_etree(
            {
                "filter": {
                    "configuration": {
                        "interfaces": {
                            "interface": {"name": "irb", "unit": {"name": "456"}}
                        }
                    }
                }
            }
    ...
    def test_filtering_should_only_consider_non_empty_text_nodes(self):
        content = dict_2_etree(
            {"data": {"configuration": {"interfaces": {"interface": {"name": "key1"}}}}}
        )
        ...
        assert_that(
            content.xpath("//data/configuration/interfaces/interface"), has_length(1)
        )
...
class XmlEqualsToMatcher(BaseMatcher):
    ...
    def compare_children(self, expected, actual):
        ...
        for node in expected:
            actual_node = get_children_by_unqualified_tag(
                unqualify(node.tag), actual, excluding=tested_nodes
            )
...
def get_children_by_unqualified_tag(tag, node, excluding):
    ...
    raise AssertionError(
        "Missing element {} in {}".format(tag, to_xml(node, pretty_print=True))
    )

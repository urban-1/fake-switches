...
class JuniperQfxCopperProtocolTest(BaseJuniper):
    ...
    def test_access_mode(self):
        ...
        assert_that(
            int003.xpath("unit/family/ethernet-switching/interface-mode")[0].text,
            equal_to("access"),
        )
        assert_that(
    ...
    def test_trunk_mode_does_not_allow_no_vlan_members(self):
        ...
        with self.assertRaises(RPCError) as context:
            ...
            assert_that(
                etree.tostring(context.exception._raw.xpath("/*/*")[0]),
                xml_equals_to(
...
configuration check-out failed
    ...
    def test_interface_set_trunk_native_vlan_then_set_members_after(self):
        ...
        self.edit(
            {
                "interfaces": {
                    "interface": [{"name": "ge-0/0/3"}, {"native-vlan-id": "2995"}]
                }
            }
        )
        self.nc.commit()
    ...
    def test_assigning_unknown_native_vlan_raises(self):
        self.edit(
            {
                "interfaces": {
                    "interface": [{"name": "ge-0/0/3"}, {"native-vlan-id": "2000"}]
                }
            }
        )
        ...
        self.edit(
            {
                ...
                "interfaces": {
                    "interface": [{"name": "ge-0/0/3"}, {"native-vlan-id": "2996"}]
                },
            }
        )
        self.nc.commit()
            ...
            filter=dict_2_etree(
                {
                    "filter": {
                        "configuration": {
                            "protocols": {"rstp": {"interface": {"name": "ge-0/0/3"}}}
                        }
                    }
                }
            ),
            ...
            filter=dict_2_etree(
                {
                    "filter": {
                        "configuration": {
                            "protocols": {"lldp": {"interface": {"name": "ge-0/0/3"}}}
                        }
                    }
                }
            ),
        ...
        self.edit(
            {
                "vlans": [{"vlan": [{"name": "VLAN2996"}, {"vlan-id": "2996"}]}],
                "interfaces": {
                    "interface": [{"name": "ge-0/0/2"}, {"native-vlan-id": "2996"}]
                },
            }
        )
    ...
    def test_set_interface_raises_on_aggregated_out_of_range_port(self):
        ...
        assert_that(
            str(exc.exception),
            contains_string("device value outside range 0..999 for '9000' in 'ae9000'"),
        )
    ...
    def test_operational_request_get_interface_information_terse(self):
        self.edit(
            {
                "vlans": [
                    {"vlan": [{"name": "VLAN1999"}, {"vlan-id": "1999"}]},
                ],
                "interfaces": [
                    {
                        "interface": [
        ...
        assert_that(
            deleted_interface.xpath("logical-interface/name")[0].text,
            is_("\nge-0/0/1.16386\n"),
        )
        assert_that(
            deleted_interface.xpath("logical-interface/admin-status")[0].text,
            is_("\nup\n"),
        )
        assert_that(
            deleted_interface.xpath("logical-interface/oper-status")[0].text,
            is_("\ndown\n"),
        )
        assert_that(
            deleted_interface.xpath("logical-interface/filter-information"),
            has_length(1),
        )
        assert_that(
            deleted_interface.xpath("logical-interface/filter-information/*"),
        ...
        assert_that(
            access_mode_interface.xpath("description")[0].text, is_("\nmy crib\n")
        )
        ...
        assert_that(
            bond_member_interface.xpath("description")[0].text, is_("\nbond member\n")
        )

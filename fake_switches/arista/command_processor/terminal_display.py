...
class TerminalDisplay(object):
    ...
    def show_vlans(self, processor, vlans_json):
        ...
        for vlan_number in sorted(vlans_json["vlans"].keys(), key=lambda e: int(e)):
            processor.write_line(
                "{: <5} {: <32} active".format(
                    vlan_number, vlans_json["vlans"][vlan_number]["name"]
                )
            )
    ...
    def show_interface(self, processor, interfaces_json):
        for interface in sorted(
            interfaces_json["interfaces"].values(),
            key=lambda e: int(split_port_name(e["name"])[1]),
        ):
    ...
    def _show_vlan_port(self, processor, interface):
        processor.write_line(
            "{} is up, line protocol is up (connected)".format(interface["name"])
        )
        processor.write_line(
            "  Hardware is Vlan, address is {} (bia {})".format(
                _mac6to3(interface["physicalAddress"]),
        ...
        if int_address is not None:
            ...
            if primary == "0.0.0.0/0":
                ...
                for secondary_ip in int_address["secondaryIpsOrderedList"]:
                    processor.write_line(
                        "  Secondary address is {}".format(_to_cidr(secondary_ip))
                    )
    ...
    def _show_phys_port(self, processor, interface):
        ...
        processor.write_line(
            "  Hardware is Ethernet, address is {} (bia {})".format(
                _mac6to3(interface["physicalAddress"]),
                _mac6to3(interface["burnedInAddress"]),
            )
        )
    ...
    def show_interface_switchport(self, processor, switchport_json):
        ...
        for name, properties in sorted(
            iter(ports), key=lambda item: int(split_port_name(item[0])[1])
        ):
            switchport = properties["switchportInfo"]
            processor.write_line("Name: {}".format(short_port_name(name)))

            processor.write_line(
                "Switchport: {}".format(
                    "Enabled" if properties["enabled"] else "Disabled"
                )
            )

            processor.write_line(
                "Administrative Mode: {}".format(
                    "static access"
                    if switchport["mode"] == "access"
                    else switchport["mode"]
                )
            )

            processor.write_line(
                "Operational Mode: {}".format(
                    "static access"
                    if switchport["mode"] == "access"
                    else switchport["mode"]
                )
            ...
            processor.write_line(
                "MAC Address Learning: {}".format(
                    "enabled" if switchport["macLearning"] else "disabled"
                )
            )

            processor.write_line(
                "Dot1q ethertype/TPID: {} ({})".format(
                    switchport["tpid"],
                    "active" if switchport["tpidStatus"] else "inactive",
                )
            )

            processor.write_line(
                "Dot1q Vlan Tag Required (Administrative/Operational): {}".format(
                    "Yes/Yes" if switchport["dot1qVlanTagRequiredStatus"] else "No/No"
                )
            )

            processor.write_line(
                "Access Mode VLAN: {} ({})".format(
                    switchport["accessVlanId"], switchport["accessVlanName"]
                )
            )

            processor.write_line(
                "Trunking Native Mode VLAN: {} ({})".format(
                    switchport["trunkingNativeVlanId"],
                    switchport["trunkingNativeVlanName"],
                )
            )
            ...
            processor.write_line(
                "Trunking VLANs Enabled: {}".format(switchport["trunkAllowedVlans"])
            )
            ...
            processor.write_line(
                "Source interface filtering: {}".format(
                    switchport["sourceportFilterMode"]
                )
            )

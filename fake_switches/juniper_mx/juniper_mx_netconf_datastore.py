...
from fake_switches.juniper.juniper_netconf_datastore import (
    ...
    parse_range,
    val,
    _restore_protocols_specific_data,
)
from fake_switches.juniper_qfx_copper.juniper_qfx_copper_netconf_datastore import (
    JuniperQfxCopperNetconfDatastore,
)
...
class JuniperMxNetconfDatastore(JuniperQfxCopperNetconfDatastore):
    ...
    def member_list_trunk_vlan_error(self, port):
        return FailingCommitResults(
            [
                TrunkShouldHaveVlanMembers(interface=port.name),
    ...
    def parse_vlan_interfaces(self, conf, interface_node):
        for unit_node in interface_node.xpath("unit/name/.."):
            ...
            inet = first(
                unit_node.xpath("family/inet".format(self.ETHERNET_SWITCHING_TAG))
            )
            if inet is not None:
                    ...
                    if resolve_operation(address) == "delete":
                        ...
                        for vrrp_node in address.xpath("vrrp-group/name/.."):
                            ...
                            if (
                                ...
                                and resolve_operation(vrrp_node) == "delete"
                            ):
                                ...
                                vrrp_group.ip_addresses = [
                                    vip.text
                                    for vip in vrrp_node.xpath("virtual-address")
                                ...
                                if (
                                    resolve_operation(first(vrrp_node.xpath("preempt")))
                                    == "delete"
                                ):
                                ...
                                if first(vrrp_node.xpath("accept-data")) is not None:
                                    if (
                                        resolve_operation(
                                            first(vrrp_node.xpath("accept-data"))
                                        )
                                        == "delete"
                                    ):
                                ...
                                if (
                                    vrrp_group.vendor_specific["authentication-type"]
                                    is None
                                ):
                                ...
                                if track is not None:
                                    if resolve_operation(track) == "delete":
                                        ...
                                        vrrp_group.track = {
                                            val(track, "route/route_address"): val(
                                                track, "route/priority-cost"
                                            )
                                        }
    ...
    def parse_vlan_attributes(self, conf, vlan, vlan_node):
        ...
        vlan.vendor_specific["linked-port-vlan"] = resolve_new_value(
            vlan_node, "routing-interface", vlan.vendor_specific.get("linked-port-vlan")
        )
    ...
    def to_irb_interfaces(self, vlan_ports):
        ...
        for vlan_port in vlan_ports:
            unit = {"name": vlan_port.vendor_specific["irb-unit"]}

            inet = []
            if vlan_port.ips:
                inet.extend(
                    [
                        {"address": self._address_etree(ip, vlan_port)}
                        for ip in vlan_port.ips
                    ]
                )
            ...
            if vrrp.related_ip_network == ip:
                ...
                for ip_address in vrrp.ip_addresses:
                    ...
                    if vrrp.preempt_delay_minimum is not None:
                        vrrp_etree.append(
                            {"preempt": {"hold-time": vrrp.preempt_delay_minimum}}
                        )
                    ...
                    if vrrp.vendor_specific.get("authentication-type") is not None:
                        vrrp_etree.append(
                            {
                                "authentication-type": vrrp.vendor_specific.get(
                                    "authentication-type"
                                )
                            }
                        )
                    ...
                    if vrrp.authentication is not None:
                        vrrp_etree.append(
                            {
                                "authentication-key": "this is {} but hashed".format(
                                    vrrp.authentication
                                )
                            }
                        )
    ...
    def vlan_to_etree(self, vlan):
        ...
        if vlan.vendor_specific.get("linked-port-vlan"):
            etree.append(
                {"routing-interface": vlan.vendor_specific.get("linked-port-vlan")}
            )
...
class TrunkShouldHaveVlanMembers(NetconfError):
    def __init__(self, interface):
        super(TrunkShouldHaveVlanMembers, self).__init__(
            msg="mgd: 'interface-mode trunk' must be defined with either "
...
class ConfigurationCheckOutFailed(NetconfError):
    def __init__(self):
        super(ConfigurationCheckOutFailed, self).__init__(
            msg="commit failed: (statements constraint check failed)",
            severity="error",
            err_type="protocol",
...
class IpAlreadyInUse(NetconfError):
    def __init__(self, message):
        super(IpAlreadyInUse, self).__init__(
            message,
            severity="error",
            err_type="protocol",

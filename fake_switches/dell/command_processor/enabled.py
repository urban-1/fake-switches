...
class DellEnabledCommandProcessor(BaseCommandProcessor):
            ...
            if len(args) == 1:
                ...
                if len(self.switch_configuration.vlans) > 0:
                    self.write_line(
                        ...
                        % ",".join(
                            sorted(
                                [str(v.number) for v in self.switch_configuration.vlans]
                            )
                        )
                    )
    ...
    def get_port_configuration(self, port):
        ...
        if port.trunk_vlans:
            conf.append(
                "switchport {} allowed vlan add {}".format(
    ...
    def get_interfaces_status_output(self):
        ...
        for port in sorted(interfaces, key=lambda e: e.name):
            output_lines.append(
                "{name: <5}  {type: <30}  {duplex: <6}  {speed: <7}  {neg: <4} {state: <9} {flow}".format(
                    ...
                    type="10G - Level" if "x" in port.name else "Gigabit - Level",
                    duplex="Full",
                    speed="Unknown",
                    neg="Auto",
                    state="Down",
        ...
        for port in sorted(bonds, key=lambda e: int(e.name.split(" ")[-1])):
            output_lines.append(
                "ch{name: <2} {type: <30}  {state}".format(
                    name=port.name.split(" ")[-1],
                    type="Link Aggregate",
                    state="Down",
                    flow="Inactive",
    ...
    def show_vlan_page(self, vlans):
        ...
        while len(vlans) > 0 and line_count < lines_per_pages:
            ...
            self.write_line(
                "{number: <5}  {name: <32} {ports: <13}  {type: <8}  {auth: <13}".format(
                    number=vlan.number,
                    name=vlan_name(vlan),
                    ports=ports_strings[0],
    ...
    def get_ports_for_vlan(self, vlan):
        ...
        for port in self.switch_configuration.ports:
            if not isinstance(port, VlanPort):
                if (
                    port.trunk_vlans and vlan.number in port.trunk_vlans
    ...
    def _build_port_strings(self, ports):
        ...
        for port_range in port_range_list:
            ...
            if len(port_range) == 1:
                port_list.append(
                    "{}{}".format(first_details.port_prefix, first_details.port)
                )
            else:
                port_list.append(
                    "{0}{1}-{0}{2}".format(
                        first_details.port_prefix,
                        first_details.port,
                        self._get_interface_details(port_range[-1].name).port,
                    )
                )
        return _assemble_elements_on_lines(port_list, max_line_char=13)

    def _get_interface_details(self, interface_name):
        interface_descriptor = namedtuple(
            "InterfaceDescriptor", "interface port_prefix port"
        )
    ...
    def _are_in_sequence(self, a, b):
        ...
        return (
            details_a.port + 1 == details_b.port
            and details_a.port_prefix == details_b.port_prefix
        )

...
class Dell10GEnabledCommandProcessor(DellEnabledCommandProcessor):
    ...
    def write_vlans(self):
        ...
        self.write_line(
            "vlan {}".format(to_vlan_ranges([v.number for v in other_vlans]))
        )
        self.write_line("exit")
            ...
            if not isinstance(port, AggregatedPort):
                self.write_line(
                    "Te{name: <7} {desc: <25} {vlan: <5} {duplex: <6} {speed: <7} {neg: <4} {state: <6} {flow}".format(
                        ...
                        desc=port.description[:25] if port.description else "",
                        vlan="",
                        duplex="Full",
                        speed="10000",
                        neg="Auto",
            ...
            if isinstance(port, AggregatedPort):
                self.write_line(
                    "Po{name: <7} {desc: <28} {vlan: <5} {state}".format(
                        name=port.name.split(" ")[-1],
                        desc=port.description[:28] if port.description else "",
                        vlan="trnk",
                        state="Up",
                    )
                )
    ...
    def show_vlans(self, vlans):
        ...
        for vlan in vlans:
            ...
            self.write_line(
                "{number: <5}  {name: <32} {ports: <13}  {type}".format(
                    number=vlan.number,
                    name=vlan_name(vlan),
    ...
    def _build_port_strings(self, ports):
        ...
        for port_range in port_range_list:
            ...
            if len(port_range) == 1:
                port_list.append(
                    "Te{}{}".format(first_details.port_prefix, first_details.port)
                )
            else:
                port_list.append(
                    "Te{0}{1}-{2}".format(
